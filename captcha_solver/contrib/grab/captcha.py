...
def solve_captcha(solver, g, url=None,  **kwargs):
    if url:
        logger.debug('Downloading captcha')
        g.request(url=url)
    data = g.response.body
    logger.debug('Solving captcha')
    solution = solver.solve_captcha(data=data, **kwargs)
...
RE_SCRIPT2 = re.\
    compile(r'<script[^>]+google\.com/recaptcha/api/challenge[^>]+>', re.S)
        ...
        for x in xrange(5):
            ...
            if not url:
                ...
                if match:
                    url = RE_SRC.search(match.group(0)).group(1)
            if not url:
                if 'google.com/recaptcha/api/js/recaptcha_ajax.js' \
                        in g.response.body:
                    ...
                    # recaptcha/api/image?c=%s' % challenge_code
            ...
            server = re.compile(r'server\s*:\s*\'([^\']+)')\
                .search(html).group(1)
            challenge = re.compile(r'challenge\s*:\s*\'([^\']+)')\
                .search(html).group(1)
            url = server + 'image?c=' + challenge
            return challenge, url
        raise CaptchaError('Could not get valid response'
                           ' from recaptcha server')
